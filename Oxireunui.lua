...
function OxireunUI:NewWindow(title)
    ...
    function Window:NewSection(name)
        ...
        function Section:CreateSlider(name, min, max, default, callback)
            ...
            -- Start drag when input begins on the slider button (capture the input object)
            SliderButton.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 then
                    sliderDragging = true
                    activeInput = input
                    -- Ensure we clear dragging when THIS input ends
                    input.Changed:Connect(function()
                        if input.UserInputState == Enum.UserInputState.End then
            ...
            -- Start drag when clicking the track (we get the input param here)
            ...
            -- Update slider while dragging (runs only when sliderDragging is true)
