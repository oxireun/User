-- [ OXIREUN ANTI-SPY PROTECTION ]
-- Remote Spy araçlarının bu UI ve executor üzerinden yapılan çağrıları loglamasını engeller.
-- Bu blok, Spy araçlarının hook'larını bypass ederek orijinal fonksiyonları doğrudan çağırır.
do
    local success, err = pcall(function()
        if getgenv and hookmetamethod and checkcaller then
            local Event = Instance.new("RemoteEvent")
            local Function = Instance.new("RemoteFunction")
            
            -- Orijinal C-Closure fonksiyonlarını al (Spy hookları olmadan)
            local OriginalFireServer = Event.FireServer
            local OriginalInvokeServer = Function.InvokeServer
            ...
            OldNameCall = hookmetamethod(game, "__namecall", newcclosure(function(Self, ...)
                ...
                -- Eğer çağrı bizim scriptten (executor) geliyorsa ve Remote tetikliyorsa
                if checkcaller() then
                    if Method == "FireServer" then
                        -- Spy hook'unu atla, direkt orijinali çalıştır
