...
-- Remote Logger Protection Entegre Edilmiş Versiyon
local OxireunUI = {}
OxireunUI.__index = OxireunUI

-- ==========================================
-- REMOTE LOGGER PROTECTION (ANTI-SPY)
-- ==========================================
local function StartRemoteProtection()
    local CoreGui = game:GetService("CoreGui")
    local PlayerGui = game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui")
    
    -- Bilinen logger isimleri
    local ForbiddenNames = {
        "Hydroxide", "SimpleSpy", "RemoteSpy", "TurtleSpy", 
        "Spy", "Logger", "RemoteExplorer", "Simple Spy"
    }

    local function CheckAndDestroy(child)
        for _, name in pairs(ForbiddenNames) do
            if child.Name:find(name) or child:FindFirstChild(name) then
                child:Destroy() -- Logger'ı anında kapatır
                -- Opsiyonel: Loglara düşmemek için bildirim gönderilebilir
            end
        end
    end

    -- CoreGui ve PlayerGui'yi izle
    CoreGui.ChildAdded:Connect(CheckAndDestroy)
    PlayerGui.ChildAdded:Connect(CheckAndDestroy)

    -- Halihazırda açık olan logger varsa kapat
    for _, child in pairs(CoreGui:GetChildren()) do CheckAndDestroy(child) end
    for _, child in pairs(PlayerGui:GetChildren()) do CheckAndDestroy(child) end

    -- Metatable Hooking (Remote trafiğini gizlemek için temel önlem)
    local oldNamecall
    oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
        local method = getnamecallmethod()
        local args = {...}
        
        -- Eğer çağrı bizim UI pathimizden geliyorsa logger'ların algılamasını zorlaştır
        if not checkcaller() and (method == "FireServer" or method == "InvokeServer") then
            local script = getcallingscript()
            if script and script:IsDescendantOf(CoreGui:FindFirstChild("OxireunUI")) then
                -- Burada remote'u manipüle etmiyoruz, sadece logger'ın bu call'u yakalamasını 
                -- bazı spy scriptlerinde engellemek için checkcaller kontrolünü geçiyoruz.
            end
        end
...
-- Korumayı başlat
task.spawn(StartRemoteProtection)
-- ==========================================
...
function Window:NewSection(name)  
    ...
    function Section:CreateDropdown(name, options, default, callback)  
        ...
        DropdownButton.MouseEnter:Connect(function() game:GetService("TweenService"):Create(DropdownButton, TweenInfo.new(0.2), {BackgroundColor3 = Colors.Border}):Play() end)  
        DropdownButton.MouseLeave:Connect(function() game:GetService("TweenService"):Create(DropdownButton, TweenInfo.new(0.2), {BackgroundColor3 = Colors.Button}):Play() end)  
        local open = false  
        ...
        DropdownButton.MouseButton1Click:Connect(function()  
            ...
            dropdownConnection = RunService.Heartbeat:Connect(function() if OptionsContainer and open then OptionsContainer.Position = UDim2.new(0, DropdownButton.AbsolutePosition.X, 0, DropdownButton.AbsolutePosition.Y + DropdownButton.AbsoluteSize.Y + 5) end end)  
            UserInputService.InputBegan:Connect(function(input)  
                if input.UserInputType == Enum.UserInputType.MouseButton1 then  
                    ...
                    if not (mousePos.X >= buttonPos.X and mousePos.X <= buttonPos.X + buttonSize.X and mousePos.Y >= buttonPos.Y and mousePos.Y <= buttonPos.Y + buttonSize.Y) and  
                       not (containerPos and containerSize and mousePos.X >= containerPos.X and mousePos.X <= containerPos.X + containerSize.X and mousePos.Y >= containerPos.Y and mousePos.Y <= containerPos.Y + containerSize.Y) then  
    ...
    function Section:CreateTextbox(name, callback)  
        ...
        InputBox.FocusLost:Connect(function() if callback then callback(InputBox.Text) end end)  
        return Textbox  
    end  
