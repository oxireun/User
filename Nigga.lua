...
-- Remote Logger Protection Enhanced Version

local OxireunUI = {}
OxireunUI.__index = OxireunUI

-- [[ REMOTE LOGGER PROTECTION SYSTEM ]] --
local function StartAntiLogger()
    local CoreGui = game:GetService("CoreGui")
    local LogKeywords = {"remote", "spy", "logger", "hydroxide", "simplespy", "event", "fire", "namecall"}
    
    local function ScanUI(gui)
        if gui == script.Parent or gui.Name == "OxireunUI" then return end
        
        local isLogger = false
        ...
        for _, descendant in pairs(gui:GetDescendants()) do
            if descendant:IsA("TextLabel") or descendant:IsA("TextBox") or descendant:IsA("TextButton") then
                ...
                -- İçerik taraması (İsimden bağımsız)
                for _, word in pairs(LogKeywords) do
                    if string.find(text, word) then
                        detectionScore = detectionScore + 1
                    end
                end
                
                -- Özel yapısal tarama (Hydroxide & SimpleSpy paternleri)
                if string.find(text, "fireserver") or string.find(text, "invokecl") or string.find(text, "args") then
                    isLogger = true
                end
            end
        end
        
        if isLogger or detectionScore >= 3 then
            gui:Destroy() -- Logger tespit edildiğinde anında yok et
            warn("[Oxireun Protection]: Remote Logger detected and terminated.")
        end
    end

    -- Mevcut GUI'leri tara
    for _, gui in pairs(CoreGui:GetChildren()) do
        ScanUI(gui)
    end

    -- Yeni eklenen GUI'leri anlık izle
    CoreGui.ChildAdded:Connect(function(child)
        task.wait(0.1) -- Yüklenmesi için kısa bir süre bekle
        ScanUI(child)
    end)
    
    -- Namecall Hook (Eğer executor destekliyorsa remote dinlemeyi bozar)
    if hookmetamethod then
        ...
        oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
            ...
            -- Eğer çağrı senin UI içinden gelmiyorsa ve bir Logger sızmaya çalışıyorsa engelle
            if not checkcaller() and (method == "FireServer" or method == "InvokeServer") then
                -- Burada remote loggers'ın yakalamasını zorlaştıran ek mantıklar çalışabilir
            end
...
function OxireunUI:NewWindow(title)
    ...
    function Window:NewSection(name)  
        ...
        function Section:CreateSlider(name, min, max, default, callback)  
            ...
            game:GetService("UserInputService").InputEnded:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 then draggingSlider = false end end)  
